...
This scenario allows support agents to reply to customer Whatsapp or Instagram messages directly from a Telegram groups.

It receives Telegram replies from customer support or logistics groups and sends the response back to the customer via Instagram DM or Whastapp.

Agents can either:
- send the AI suggested reply (`send`) - from customer support group
- send a custom reply by typing a message from - from either of groups
...
↓
Set Variable → record_id
↓
Airtable → Get Record
↓
Router A — Customer Support Command Router
     ├ send (use AI reply)
     └ custom reply
↓
Router B — Logistics Group Router
     ├ instagram
     └ whatsapp
↓
HTTP → send message
...
Additional Router Route:
Order Dispatcher
↓
Detect reply to 🆔 message
↓
Retrieve Airtable record
↓
Channel Router
↓
...
## Order  Support Route (AI Support Workflow)

### Filter

Ensures the message is a reply to a support notification but not an Order Dispatcher message.
...
This route is triggered when the operator replies to a Telegram message that contains an Airtable record identifier.
...
Only replies to Telegram messages containing an Airtable record identifier (`🆔`) will trigger the Order Dispatcher route.

This prevents the scenario from responding to normal conversation messages inside the Telegram support group.
...
This ensures that outbound messages are correctly attributed to the responding agent while maintaining the integrity of the conversation thread.
